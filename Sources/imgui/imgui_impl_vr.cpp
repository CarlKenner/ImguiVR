...
static const char* ImGui_Impl_VR_GetClipboardText(void* user_data)
{
	//NateVM - Added user_data for 2016 imgui api change. Not sure about openvr equivalent...
	//return SDL_GetClipboardText(); //any openvr equivalent here?
	return 0;
}

static void ImGui_Impl_VR_SetClipboardText(void* user_data, const char* text)
{
...
void ImGui_Impl_VR_ProcessEvent(vr::VREvent_t* event)
{
	...
	if(event->trackedDeviceIndex == left_controller) //right controller is mouse
	{
		...
		if(controller.button == vr::k_EButton_Axis0)
		{
			...
			if( g_HMD->GetControllerState( left_controller, &state, sizeof(state) ) )
			{
	...
	if(event->trackedDeviceIndex == right_controller) //right controller is mouse
	{
		...
		if(controller.button == vr::k_EButton_Axis0)
		{
			...
			vr::VRControllerState_t state;
			if( g_HMD->GetControllerState( right_controller, &state, sizeof(state) ) )
			{
